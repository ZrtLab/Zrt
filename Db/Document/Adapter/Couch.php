...
class Zrt_Db_Document_Adapter_Couch
    {
    ...
    public function setDb( $db )
        ...
        if ( !preg_match( '/^[a-z][a-z0-9_$()+-\/]+$/' , $db ) )
            {
            throw new Zrt_Exception( sprintf( 'Invalid database specified: "%s"' ,
                                              htmlentities( $db ) ) );
            }
        $this->_config['db'] = $db;
        $this->_dirty = true;
     ...
     * Queries a view.
     *
     * @param Zrt_Db_Document_View
     * @return Zrt_Db_Document_CouchSet
     */
    public function view( Zrt_Db_Document_View $view )
        {
        $db = $this->getDb();
        $designDocument = $view->getDesignDocument();
        $name = $view->getName();
        $parameters = $view->getParameters();
        $response = $this->_prepare( "/$db/_design/$designDocument/_view/$name" ,
                                     $parameters )->_execute();
        ...
        switch ( $status )
            {
            case Zrt_Http::OK:
                return new Zrt_Db_Document_CouchSet( array(
                            'adapter' => $this ,
            ...
            case Zrt_Http::NOT_FOUND:
                throw new Zrt_Exception( "$name is not a valid view in the $designDocument document." );
                break;

            case Zrt_Http::BAD_REQUEST:
                // Usually a JSON formatting error.
                throw new Zrt_Exception( "Request was poorly formatted" );
                break;
    ...
    public function save( Zrt_Db_Document_Couch $document ,
        ...
        switch ( $status )
            {
            case Zrt_Http::OK:
            case Zrt_Http::CREATED:

                $decodedResponse = Zend_Json::decode( $response->getBody() );
                $revision = $decodedResponse['rev'];
                break;

            case Zrt_Http::PRECONDITION_FAILED:
                throw new Zrt_Exception( "A document with the ID $id already exists" );
                break;

            case Zrt_Http::CONFLICT:
                throw new Zrt_Exception( "There was a conflict updating the database" );
                break;
            ...
            default:
                throw new Zrt_Exception( "Response code $status not handled." . $response->getBody() );
                break;
            }
        return $revision;
    ...
    protected function _getBaseUri()
        ...
        if ( $username )
            ...
            if ( !$password )
                {
                throw new Zrt_Exception( "Must specify both a username and password for authentication." );
                }
