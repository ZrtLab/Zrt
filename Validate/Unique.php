...
class Zrt_Validate_Unique
     ...
     */
    public function __construct( Zrt_Model $model , $field , $identifier = null )
        {
        $this->setModel( $model );
        $this->setField( $field );
        if ( null !== $identifier )
    ...
    public function isValid( $value , $context = null )
        ...
        if ( !$context )
            {
            throw new Zrt_Exception( "Cannot test for uniqueness with no context" );
            }

        if ( null === $this->_model )
            {
            throw new Zrt_Exception( "No model specified for uniqueness check." );
            }

        if ( null === $this->_field )
            {
            throw new Zrt_Exception( "No field specified for uniqueness check." );
            }

        $valueString = ( string ) $value;
        $this->_setValue( $valueString );

        $select = $this->_model->select()->where( "$this->_field = ?" ,
                                                  $valueString );
        if ( isset( $context['id'] ) )
            {
