...
class App_View_Helper_MinifyHeadLink extends Zend_View_Helper_HeadLink
{
     ...
     * @var string
     */
    protected $_minifyLocation = '/static/min/';
    ...
    public function toString($indent = null)
    {
        ...
        foreach ($this as $item) {
            if ($item->type == 'text/css' && $item->conditionalStylesheet === false && strpos($item->href,
                    'http://') === false && $this->isValidStyleSheetExtension($item->href)) {
                $stylesheets [$item->media] [] = str_replace($this->getBaseUrl(),
                    '', $item->href);
            } else {
                ...
                foreach ($stylesheets as $media => $styles) {
                    ...
                    $minStyles->href = $this->getMinUrl() . '?f=' . implode(',',
                            $styles);
                    if ($trimmedBaseUrl)
