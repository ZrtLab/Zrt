...
//	protected $_regKey = 'RC_View_Helper_MinifyHeadScript';
     ...
     */
    public function minifyHeadScript($mode = Zend_View_Helper_HeadScript::FILE,
    ...
    protected function _generateMinifyItem(array $scripts)
    {
        ...
        if (is_null($baseUrl) || $baseUrl == '') {
            $minScript->attributes['src'] = $this->getMinUrl() . '?f=' . implode(',',
                    $scripts);
        } else {
            $minScript->attributes['src'] = $this->getMinUrl() . '?b=' . $baseUrl . '&f=' . implode(',',
                    $scripts);
        }
