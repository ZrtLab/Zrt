...
class Zrt_Debug_ChromePhp
    ...
    private function __construct()
        ...
        $this->_timestamp = $this->_php_version >= 5.1 ? $_SERVER['REQUEST_TIME']
                    : time();
    ...
    public static function getInstance()
        ...
        if ( self::$_instance === null )
            {
            self::$_instance = new Zrt_Debug_ChromePhp();
            }
    ...
    protected function _convert( $object )
        ...
        foreach ( $object_vars as $key => $value )
            ...
            if ( $value === $object || in_array( $value , $this->_processed ,
                                                 true ) )
                {
    ...
    protected function _setCookie( $data )
        {
        return setcookie( self::COOKIE_NAME , $this->_encode( $data ) ,
                                                              time() + 30 );
        }
    ...
    protected function _writeToFile( $json )
        ...
        file_put_contents( $this->getSetting( self::LOG_PATH ) . '/' . $file_name ,
                                              $json );
