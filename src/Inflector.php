 ...
 *
 * @defgroup Zrt_Inflection Zrt Inflection
 */
...
class Zrt_Inflector
    ...
    public static function rules( $type , $rules = array( ) )
        ...
        foreach ( $rules as $rule => $pattern )
            ...
            if ( is_array( $pattern ) )
                {
                self::${$type}[$rule] = array_merge( $pattern ,
                                                     self::${$type}[$rule] );
                unset( $rules[$rule] , self::${$type}['cache' . ucfirst( $rule )] ,
                                                                         self::${$type}['merged'][$rule] );
                }
            }
    ...
    public static function pluralize( $word )
        {
        $_this = Zrt_Inflector::getInstance();
        ...
        if ( !isset( self::$_plural['merged']['uninflected'] ) )
            {
            self::$_plural['merged']['uninflected'] = array_merge( self::$_plural['uninflected'] ,
                                                                   self::$_uninflected );
            }

        if ( !isset( self::$_plural['cacheUninflected'] ) || !isset( self::$_plural['cacheIrregular'] ) )
            {
            self::$_plural['cacheUninflected'] = '(?:' . join( '|' ,
                                                               self::$_plural['merged']['uninflected'] ) . ')';
            self::$_plural['cacheIrregular'] = '(?:' . join( '|' ,
                                                             array_keys( self::$_plural['merged']['irregular'] ) ) . ')';
            }

        if ( preg_match( '/(.*)\\b(' . self::$_plural['cacheIrregular'] . ')$/i' ,
                         $word , $regs ) )
            {
            self::$_pluralized[$word] = $regs[1] . substr( $word , 0 , 1 ) . substr( self::$_plural['merged']['irregular'][strtolower( $regs[2] )] ,
                                                                                                                                       1 );
            return self::$_pluralized[$word];
            }

        if ( preg_match( '/^(' . self::$_plural['cacheUninflected'] . ')$/i' ,
                         $word , $regs ) )
            {
        ...
        foreach ( self::$_plural['rules'] as $rule => $replacement )
            ...
            if ( preg_match( $rule , $word ) )
                {
                self::$_pluralized[$word] = preg_replace( $rule , $replacement ,
                                                          $word );
                return self::$_pluralized[$word];
                }
            }
    ...
    public static function singularize( $word )
        ...
        if ( !isset( self::$_singular['merged']['uninflected'] ) )
            {
            self::$_singular['merged']['uninflected'] = array_merge( self::$_singular['uninflected'] ,
                                                                     self::$_uninflected );
            }

        if ( !isset( self::$_singular['merged']['irregular'] ) )
            {
            self::$_singular['merged']['irregular'] = array_merge( self::$_singular['irregular'] ,
                                                                   array_flip( self::$_plural['irregular'] ) );
            }

        if ( !isset( self::$_singular['cacheUninflected'] ) || !isset( self::$_singular['cacheIrregular'] ) )
            {
            self::$_singular['cacheUninflected'] = '(?:' . join( '|' ,
                                                                 self::$_singular['merged']['uninflected'] ) . ')';
            self::$_singular['cacheIrregular'] = '(?:' . join( '|' ,
                                                               array_keys( self::$_singular['merged']['irregular'] ) ) . ')';
            }

        if ( preg_match( '/(.*)\\b(' . self::$_singular['cacheIrregular'] . ')$/i' ,
                         $word , $regs ) )
            {
            self::$_singularized[$word] = $regs[1] . substr( $word , 0 , 1 ) . substr( self::$_singular['merged']['irregular'][strtolower( $regs[2] )] ,
                                                                                                                                           1 );
            return self::$_singularized[$word];
            }

        if ( preg_match( '/^(' . self::$_singular['cacheUninflected'] . ')$/i' ,
                         $word , $regs ) )
            {
        ...
        foreach ( self::$_singular['rules'] as $rule => $replacement )
            ...
            if ( preg_match( $rule , $word ) )
                {
                self::$_singularized[$word] = preg_replace( $rule ,
                                                            $replacement , $word );
                return self::$_singularized[$word];
                }
            }
