 ...
 *
 * @defgroup Zrt_Record Zrt Record
 */
...
abstract class Zrt_Record
    ...
    final public function __construct( $options = array( ) )
        ...
        $this->addElementPrefixPath( 'Zrt_Record_Decorator' ,
                                     'Zrt/Record/Decorator' , 'decorator' );
        $this->addPrefixPath( 'Zrt_Record_Decorator' , 'Zrt/Record/Decorator' ,
                              'decorator' );
        ...
        if ( !$this->getDisplayGroup( 'main' ) )
            {
            $this->addDisplayGroup( $this->getElementNames() , 'main' ,
                                    array(
                'order' => 0 ,
    ...
    public function addElementDecorators( array $decorators )
        ...
        foreach ( $this->_getReferencedFields( true ) as $field => $reference )
            {
            $this->_elements[$field]->addDecorator( 'LookupReference' ,
                                                    array( 'service' => $reference ) );
            }
    ...
    public function populate( array $values )
        ...
        $service = $this->_service;
        $this->_editUrl = Zrt_Application::tenantUrl() . '/' . $service::getPrimaryLocation() . '/' . $values['id'] . '/edit/';
    ...
    protected function _setupElements( array $options = null )
        ...
        foreach ( $fields as $field => $definition )
            {
            $element = $this->addElement( new Zrt_Form_Element_Display( $field ) )->getElement( $field );
